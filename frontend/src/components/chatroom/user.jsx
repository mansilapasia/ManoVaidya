...
import SessionForm from './bookingForm';
...
function getAudioElement(mediaElement, config) {
    ...
    muteAudio.style['border-radius'] = '35px';
    ...
    stop.style['margin-left'] = '10px';
    stop.style['border-radius'] = '35px';
    ...
    function adjustControls() {
        console.log(mediaElementContainer.clientWidth + "  " + mediaControls.clientWidth);
        mediaControls.style.marginLeft = '100px';//(mediaElementContainer.clientWidth - (3 * mediaControls.clientWidth)) + 'px';
...
export default function UserChatroom(props) {
    const classes = useStyles();
    const [emailid, setEmail] = useState({ emailid: props.emailid });
    const [content, setContent] = useState({ content: 3 });
    ...
    const joinCall = (event) => {
        ...
        connection.openOrJoin(event.currentTarget.id);//('1jd6umj3ytz');
    ...
    async function putTherapist(data) {
        //await getUserSessions();
        await setTherapist(data);
        await setContent(1);
        //await console.log(data);
    }
    async function noTherapistFound() {
        await setTherapist('');
        await setContent(1);
    }
    const showform = (event) => {
        ...
        const url = window.location.href + "signup/gettherapist";
    ...
    async function showsessions(event) {
        await getUserSessions();
        ...
        return (
            sessions.sessions.sessions.map((session) => (
                <Grid container item xs={16} spacing={3}>
                    <React.Fragment>
                        <Grid item xs={4}>
                            <Paper className={classes.paper}>Therapist Anonyous Name: {randomName()}</Paper>
                        </Grid>
                        <Grid item xs={2}>
                            <Paper className={classes.paper}>Date: {session.date}/{session.month}/{session.year}</Paper>
                        </Grid>
                        <Grid item xs={2}>
                            <Paper className={classes.paper}>Time: {session.time}:00 HRS</Paper>
                        </Grid>
                        <Grid item xs={2}>
                            <Paper className={classes.paperbtn} id={session.roomid} data-name={randomName()} onClick={joinCall}>Join Now!</Paper>
                        </Grid>
                    </React.Fragment>
                </Grid>
            ))
    ...
    //    const stylecsspath = 'frontend/src/components/chatroom/controlbox.css';
    return (
        <div>
            {props.usertype == 1 ? (
                        ...
                        <Grid item xs={3}>
                            <Grid container spacing={2}>
                                <Grid item xs={12} className={classes.griditem}>
                                    <Button className={classes.paper} onClick={showform}>Book a session</Button>
                                </Grid>
                                <Grid item xs={12} className={classes.griditem}>
                                    <Button className={classes.paper} onClick={showsessions}>Upcoming sessions</Button>
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item xs={9}>
                            {console.log(therapist)}
                            {content === 1 ? <div><SessionForm therapist={therapist} emailid={emailid.emailid} /></div> : (
                                content === 2 ? <Grid container spacing={1}> {displaySession()}</Grid> : null)}
                        </Grid>
                    </Grid>
                </div>
            ) : (<div className={classes.root}><TherapistChatroom connection={connection} emailid={props.emailid} /></div>)}
            <div id="audios-container" className={classes.spacing}></div>
        </div >
    );
}
//Hello User
